...
Rerpoduction, considered as meaningful and useful in the scientific community, becomes a hot topic and also on the rise. However, only a few studies could be reproduced directly. One of biggest issues is the lack of established reproducible workflows.

In doing so, I use the the Preston Curve as a case study. 
...
lat_tab[7:10] <- str_replace(
  lat_tab[7:10], fixed(smp_def$var_name[8:11]), smp_def$label[8:11]
)
...
tab[12] <- paste(
  "\\\\[-1.8ex] & \\multicolumn{4}{c}{", smp_def$label[10], "} \\\\"
)
esc_var_names <- str_replace_all(smp_def$var_name, fixed ("_"), "\\_")
reg_names <- tibble(
  var_name = esc_var_names[c(11, 9, 8)],
  label = smp_def$label[c(11, 9, 8)]
)
for (i in c(15, 18, 21)) {
  pos <- (i-12)/3
  tab[i] <- str_replace(
...
This reproduced work is merely aimed at showing the reproducible workflow and does not focus on the underlying causality.
